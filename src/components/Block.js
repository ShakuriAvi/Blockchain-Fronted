...
const Block = ({
  ...
  const [prevHash, setPrevHash] = useState(initPrevHash ? initPrevHash : "");
  const [hash, setHash] = useState(initHash ? initHash : "");

  const [loading, setLoading] = useState(false);

  const [tokens, setTokens] = useState(initTx ? initTx : "");
  const [coin, setCoins] = useState(initCoin ? initCoin : "");
